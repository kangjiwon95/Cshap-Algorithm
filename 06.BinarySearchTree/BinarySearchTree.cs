...
namespace _06.BinarySearchTree
{
    ...
    internal class BinarySearchTree<T> where T : IComparable<T>
    {
        ...
        {
            this.root = null;          // 노드의 값 구현
        }

        public bool Add(T item)  //노드를 집어넣는 구현공식
        {
        ...
        public bool Remove(T item)  // 노드값 삭제하는 기능
        {
        ...
        private Node FindNode(T item)     // 노드를 찾는 구현공식
        {
            if (root == null)
                return null;
        ...
        private void EraseNode(Node node)   //노드를 삭제할때의 자식의 유무 여하에 따라
        {                                   // 삭제후 구현공식
            if (node.HasNoChild)
            {
                if (node.IsLeftChild)
